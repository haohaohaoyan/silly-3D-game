...
func _ready():
	...
	$BottomHUD/VBoxContainer/CheckpointAnnouncement.modulate.a = 0
...
func _physics_process(_delta):
	...
	$BottomHUD/VBoxContainer/Speedometer.text = str((round(Vector2($Player.velocity.x, $Player.velocity.z).length()))) + " m/s"
	
func show_checkpoint():
	$BottomHUD/VBoxContainer/CheckpointAnnouncement.modulate.a = 1
	var fade_out = create_tween()
	fade_out.tween_property($BottomHUD/VBoxContainer/CheckpointAnnouncement, "modulate:a", 0, 3)
